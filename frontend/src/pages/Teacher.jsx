...
const Teacher = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editingTeacher) {
                // UPDATE
                await axios.put(`${API_URL}/${editingTeacher.id}`, formData, axiosConfig);
                ...
                <table className="w-full text-left">
                    <thead>
                        <tr className="bg-gray-50/50">
                            <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase">Mã GV</th>
                            <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase">Họ và Tên</th>
                            <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase">Email</th>
                            <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase">Thao tác</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-100">
                        {loading ? (
                            ...
                            <tr key={t.id} className="hover:bg-gray-50 transition-colors">
                                <td className="px-6 py-4 font-medium text-indigo-600">{t.teacher_code}</td>
                                <td className="px-6 py-4 font-semibold text-gray-900">{t.full_name}</td>
                                <td className="px-6 py-4 text-gray-600">{t.email}</td>
                                <td className="px-6 py-4 space-x-3">
                                    <button onClick={() => openModal(t)} className="text-blue-600 hover:underline">Sửa</button>
                                    <button onClick={() => handleDelete(t.id)} className="text-red-600 hover:underline">Xóa</button>
                                </td>
                            </tr>
                        ))}
                    </tbody>
            ...
            {showModal && (
                <div className="fixed inset-0 bg-black/50 flex justify-center items-center z-50">
                    <div className="bg-white p-8 rounded-2xl w-full max-w-md shadow-2xl">
                        <h2 className="text-xl font-bold mb-4">{editingTeacher ? 'Sửa Giảng Viên' : 'Thêm Giảng Viên'}</h2>
                        <form onSubmit={handleSubmit} className="space-y-4">
                            <input 
                                name="teacher_code" placeholder="Mã Giảng Viên (MSGV)" 
                                value={formData.teacher_code} onChange={handleChange}
                                className="w-full border p-2 rounded-lg outline-none focus:ring-2 focus:ring-indigo-300" required 
                            />
                            <input 
                            ...
                            <div className="flex justify-end space-x-3 mt-6">
                                <button type="button" onClick={() => setShowModal(false)} className="bg-gray-200 px-4 py-2 rounded-lg font-medium">Hủy</button>
                                <button type="submit" className="bg-indigo-600 text-white px-4 py-2 rounded-lg font-medium">Lưu lại</button>
                            </div>
                        </form>
                    </div>
                </div>

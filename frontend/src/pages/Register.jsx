...
const Regis_API_URL = `${BASE_URL}/register`;
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.status === 200 || response.status === 201) {
        ...
        alert(
          `Đăng ký thành công! Mã số của bạn là: ${assignedCode}. Hãy dùng mã này hoặc Email để đăng nhập nhé.`,
        );
        navigate("/login");
      }
    } catch (error) {
      ...
      const errorMsg =
        error.response?.data?.message || "Không kết nối được Backend.";

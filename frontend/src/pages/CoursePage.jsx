...
function CoursePage() {
  ...
  const fetchCourses = useCallback(() => {
    setLoading(true);
    axios
      .get(COURSE_API_URL, { headers: getAuthHeader() })
      .then((res) => {
  ...
  const deleteCourse = (id) => {
    if (window.confirm("Bạn có chắc muốn xóa?")) {
      axios
        .delete(`${COURSE_API_URL}/${id}`, { headers: getAuthHeader() })
        .then(() => {
          fetchCourses();
          alert("Đã xóa!");
        })
      ...
      <section className="bg-white/80 backdrop-blur-md p-6 rounded-3xl border border-gray-100 shadow-xl">
        ...
        >
          ...
          <div className="flex items-end">
            <button
              type="submit"
              className="w-full bg-purple-600 text-white font-bold py-3 rounded-xl shadow-lg"
            >
              Thêm
            </button>
          </div>
